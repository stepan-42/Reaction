...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text('Привет! Я бот для автоматической выставки реакций на истории. Выберите опцию:', reply_markup=reply_markup)
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'example':
        ...
        keyboard = [[InlineKeyboardButton("Вернуться в главное меню", callback_data='menu')]]
        reply_markup = InlineKeyboardMarkup(keyboard)
        await query.edit_message_text(text="Тарифы:\n1 месяц - цена\n3 месяца - цена\n9 месяцев - цена\n1 год - цена", reply_markup=reply_markup)
